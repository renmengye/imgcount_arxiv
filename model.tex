...
architecture (See Figure~\ref{fig:model_arch}): $x \in \mathbb{R}^{H \times W}$
...
in the Supp. Material.  We denote $D_0(x)$ as the pretrained FCN applied to the
original image.
...
\vspace{-3pt}
\begin{align}
(g_{X,Y}, \log \delta_{X,Y}, \log \sigma_{X,Y}, \gamma) &= 
W(z_{t, \text{end}}), \ \ \ \
\mu_X, \mu_Y = \frac{2g_X}{W} - 1, \frac{2g_Y}{H} - 1
\end{align}
\vspace{-16pt}

\textbf{Extracting sub-region}. We follow DRAW \cite{gregor15draw} and use a
...
\textbf{Matching IoU loss (mIOU)}. A primary challenge of instance segmentation
involves matching model and ground-truth instances.  we compute a maximum-
weighted bipartite graph matching between the output instances and ground-truth
instances \cite{stewart15lstmdet, romeraparedes15ris}. Matching makes
...
\textbf{Soft box IoU loss}. Although the exact IoU can be derived from the 4-d
...
\textbf{Monotonic score loss}. To facilitate automatic termination , the
